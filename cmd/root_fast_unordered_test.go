...
func TestRunRootCmdNoSortMaxVideosCapsOutputAndSummary(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = io.WriteString(w, `{"meta":{"status":200},"data":{"items":[{"essential":{"id":"sm1","registeredAt":"2024-01-10T00:00:00Z","count":{"comment":10}}},{"essential":{"id":"sm2","registeredAt":"2024-01-11T00:00:00Z","count":{"comment":10}}},{"essential":{"id":"sm3","registeredAt":"2024-01-12T00:00:00Z","count":{"comment":10}}}]}}`)
	}))
	t.Cleanup(server.Close)
	cfg := testFetchConfig(server.URL)
	cfg.NoSortOutput = true
	...
	if got := sortedLines(out.String()); len(got) != 2 {
		t.Fatalf("expected two output lines, got %q", out.String())
	}
...
func TestRunRootCmdNoSortMaxVideosCancelsRemainingFetches(t *testing.T) {
	slowRequestStarted := make(chan struct{})
	slowRequestCanceled := make(chan struct{})
	var slowRequestStartedOnce sync.Once
	var slowRequestCanceledOnce sync.Once
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		if strings.Contains(r.URL.Path, "/users/1/") {
			slowRequestStartedOnce.Do(func() { close(slowRequestStarted) })
			<-r.Context().Done()
			slowRequestCanceledOnce.Do(func() { close(slowRequestCanceled) })
			return
		}
		if r.URL.Query().Get("page") != "1" {
			_, _ = io.WriteString(w, `{"meta":{"status":200},"data":{"items":[]}}`)
