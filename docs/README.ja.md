...
- `--no-sort` を指定すると速度優先で出力ID一覧のソートを省略します。任意の重複除外後、IDは入力target順で、各target内の取得順を保って出力されます。
...
3. GitHub Actions がリリースワークフローを実行します（`go mod tidy`/`go generate ./...` の検証、gofmt/go vet/golangci-lint/go test/go test -race、third-party notices の同期チェック）。
